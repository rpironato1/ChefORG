...
class MCPPlaywrightOrchestrator {
  ...
  private async monitorApplicationPerformance(): Promise<any> {
    ...
    return {
      ...
      performance_score: metrics.routes_tested.length > 0 ? 
        metrics.routes_tested.reduce((sum, route) => sum + route.performance_score, 0) / metrics.routes_tested.length : 0
    };
  }
...
test.describe('MCP Playwright - Complete Test Suite', () => {
  ...
  test('Route Discovery and Performance', async () => {
    ...
    try {
      ...
      console.log(`Discovered ${routes.length} routes, ${performantRoutes.length} with good performance`);
    } finally {
      await browser.close();
    }
  });
