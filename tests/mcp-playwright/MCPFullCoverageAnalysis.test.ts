...
test.describe('MCP 100% Coverage Analysis', () => {
  test('Complete Route Discovery and Coverage', async ({ page }) => {
    const routes = {
      public: ['/', '/menu', '/reserva', '/sprint3-demo'],
      client: [
        '/checkin',
        '/chegada-sem-reserva',
        '/mesa/1/pin',
    ...
    for (const [category, files] of Object.entries(fileCategories)) {
      ...
      for (const file of files) {
        ...
        if (functionCount > 0 || componentCount > 0) {
          console.log(
            `  📄 ${file.replace(srcPath, '')} - ${functionCount} functions, ${componentCount} components`
          );
        }
      }
    }
  ...
  test('Business Logic Function Coverage', async ({ page }) => {
    ...
    const hasData = await page.evaluate(() => {
      ...
      const dataElements = document.querySelectorAll(
        '[data-testid*="data"], .data-display, .stats'
      );
  ...
  test('Mobile and Responsive Coverage', async ({ page }) => {
    ...
    for (const viewport of viewports) {
      ...
      const mobileNavExists = await page
        .locator('[data-testid*="mobile"], .mobile-nav, .drawer')
        .isVisible()
        .catch(() => false);
...
function countFunctions(content: string): number {
  const functionRegex =
    /(?:function\s+\w+|const\s+\w+\s*=\s*(?:\(.*?\)\s*=>|\w+\s*=>\s*\{)|export\s+(?:const|function)\s+\w+)/g;
  return (content.match(functionRegex) || []).length;
}

function countComponents(content: string): number {
  const componentRegex =
    /(?:const\s+[A-Z]\w*\s*=\s*(?:\(.*?\)\s*=>|React\.FC)|function\s+[A-Z]\w*\s*\(|export\s+(?:default\s+)?(?:const|function)\s+[A-Z]\w*)/g;
...
function extractFunctionNames(content: string): string[] {
  ...
  return Array.from(names).filter(
    name => name.length > 1 && !['if', 'for', 'while', 'const', 'let', 'var'].includes(name)
  );
}
...
async function validateWCAGCompliance(page: any, route: string): Promise<void> {
  try {
    ...
    console.log(
      `    WCAG ${route}: lang=${hasLangAttr}, headings=${headingCount}, alt-texts=${altTextsCount}/${allImagesCount}`
    );
  } catch (error) {
