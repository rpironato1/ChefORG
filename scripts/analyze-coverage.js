...
function analyzeRoutes() {
  ...
  const categorizedRoutes = {
    ...
    client: routes.filter(
      r => r.startsWith('/checkin') || r.startsWith('/chegada') || r.startsWith('/mesa')
    ),
...
function calculateTrueCoverage() {
  ...
  console.log(
    `  Routes: ${routeCoverage.toFixed(1)}% (${analysis.routes.tested}/${analysis.routes.total})`
  );
  console.log(
    `  Files: ${fileCoverage.toFixed(1)}% (${analysis.files.tested}/${analysis.files.total})`
  );
  console.log(
    `  APIs: ${apiCoverage.toFixed(1)}% (${analysis.apis.tested}/${analysis.apis.total})`
  );
...
function runAnalysis() {
  ...
  console.log(
    `✅ Required for 100%: ${(100 - trueCoverage).toFixed(1)}% additional coverage needed`
  );
