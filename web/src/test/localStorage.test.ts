...
describe('LocalStorageClient', () => {
  ...
  describe('Basic CRUD Operations', () => {
    ...
    it('should update data correctly', async () => {
      // Insert test data
      const userData = { nome: 'Original Name', email: 'test@example.com', role: 'cliente' as const };
      const insertResult = await client.from('users').insert(userData);
      const userId = insertResult.data?.[0].id;

      // Update data
      const updateResult = await client.from('users')
  ...
  describe('Authentication Methods', () => {
    beforeEach(() => {
      ...
      const testUsers = [
        { id: 1, nome: 'Test User', email: 'test@example.com', role: 'cliente' }
      ];
  ...
  describe('Data Persistence', () => {
    it('should persist data across client instances', async () => {
      ...
      const userData = { nome: 'Persistent User', email: 'persist@test.com', role: 'cliente' as const };
  ...
  describe('Complex Queries', () => {
    beforeEach(async () => {
      ...
      const menuItems = [
        { id: 1, nome: 'Pizza Margherita', preco: 25.00, categoria_id: 1, ativo: true },
        { id: 2, nome: 'Pizza Pepperoni', preco: 30.00, categoria_id: 1, ativo: true },
        { id: 3, nome: 'Hambúrguer', preco: 20.00, categoria_id: 2, ativo: false }
      ];
    ...
    it('should handle order by queries', async () => {
      const result = await client.from('menu_items')
        .select('*')
        .order('preco', { ascending: false });
      
      expect(result.data).toHaveLength(3);
      expect(result.data?.[0].preco).toBe(30.00); // Highest price first
    });
    ...
    it('should handle range operations', async () => {
      ...
      expect(result.data?.every(item => item.preco >= 25.00 && item.preco <= 30.00)).toBe(true);
    });
  ...
  describe('Migration Readiness', () => {
    ...
    it('should handle all table types defined in Database schema', async () => {
      ...
      const tables = [
        'users', 'tables', 'menu_items', 'menu_categories', 
