    ...
    <div className={`bg-white rounded-lg shadow-sm border transition-all duration-200 ${
      ...
      {item.imagem && (
        <div className="relative">
          <LazyImage 
            src={item.imagem} 
            alt={`Imagem do prato ${item.nome} - deliciosa opção do nosso cardápio`}
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-start mb-2">
          <h3 className={`text-lg font-semibold ${!item.disponivel ? 'text-gray-500' : 'text-gray-900'}`}>
            {item.nome}
          </h3>
          <span className={`text-xl font-bold ${!item.disponivel ? 'text-gray-500' : 'text-primary-600'}`}>
