...
export const useLoyalty = () => {
  ...
  const adicionarCliente = useCallback(
    (
      dados: Omit<ClienteFidelidade, 'id' | 'pontos' | 'nivel' | 'dataCadastro' | 'totalGasto'>
    ): ClienteFidelidade => {
  ...
  const adicionarPontos = useCallback(
    (clienteId: string, pontos: number, motivo: string, pedidoId?: string): boolean => {
  ...
  const resgatarPontos = useCallback(
    (clienteId: string, pontos: number, motivo: string): boolean => {
      const cliente = clientes.find(c => c.id === clienteId);
  ...
  const usarCupom = useCallback(
    (
      codigo: string,
      pedidoId: string
    ): { sucesso: boolean; cupom?: CupomDesconto; erro?: string } => {
  ...
  const obterCuponsCliente = useCallback(
    (clienteId: string, apenasValidos: boolean = true): CupomDesconto[] => {
      let cuponsCliente = cupons.filter(c => c.clienteId === clienteId);

      if (apenasValidos) {
        cuponsCliente = cuponsCliente.filter(c => !c.usado && new Date() <= c.validade);
      }
  ...
  const obterHistoricoCliente = useCallback(
    (clienteId: string, limite?: number): HistoricoPontos[] => {
