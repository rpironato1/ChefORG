    ...
    (set, get) => ({
      ...
      updatePedido: (id, data) =>
        set((state) => ({
          ...
          carrinhoAtual: state.carrinhoAtual?.id === id 
      ...
      getPedidosByStatus: (status) =>
        get().pedidos.filter(p => p.status === status),
      
      getPedidosByMesa: (mesaId) =>
        get().pedidos.filter(p => p.mesaId === mesaId),
      
      getActiveOrders: () =>
        get().pedidos.filter(p => 
          ['confirmado', 'preparando', 'pronto'].includes(p.status)
        )
    }),
    {
      name: 'cheforg-orders',
