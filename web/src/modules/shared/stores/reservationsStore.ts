...
export const useReservationsStore = create<ReservationsState>()(
  persist(
    (set, get) => ({
      ...
      getReservasByStatus: status => get().reservas.filter(r => r.status === status),

      getReservasByDate: date =>
        get().reservas.filter(r => r.dataHora.toDateString() === date.toDateString()),
    }),
    {
...
export const useTablesStore = create<TablesState>()(
  persist(
    (set, get) => ({
      ...
      getMesasByStatus: status => get().mesas.filter(m => m.status === status),

      getAvailableTables: capacidade => {
        const mesas = get().mesas.filter(m => m.status === 'livre');
        return capacidade ? mesas.filter(m => m.capacidade >= capacidade) : mesas;
      },
    }),
    {
      name: 'cheforg-tables',
