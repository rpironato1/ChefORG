        ...
        <div className="mb-6">
          <div className="relative max-w-md mb-4">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {category.menu_items.map(item => {
                ...
                const menuItem: MenuItem = {
                  ...
                  // Include database fields to match type
                ...
                return (
                  <CardMenuItem
                    key={item.id}
                    item={menuItem}
                    quantidade={getQuantidadeNoCarrinho(Number(item.id))}
                    onAdicionar={item => adicionarItem({ ...menuItem, id: Number(item.id) })}
                    onRemover={item => removerItem({ ...menuItem, id: Number(item.id) })}
                    showControles={true}
                  />
                );
              })}
