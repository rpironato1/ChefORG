...
function AcompanharPedido() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'preparando': return 'text-yellow-600 bg-yellow-100';
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'confirmado': return <Clock className="h-5 w-5" />;
      case 'preparando': return <ChefHat className="h-5 w-5" />;
      case 'pronto': return <Bell className="h-5 w-5" />;
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white shadow-sm border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link to={`/mesa/${numeroMesa}/cardapio`} className="flex items-center text-gray-600 hover:text-gray-900 mr-4">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6 text-center">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${getStatusColor(pedido.status)} mb-4`}>
