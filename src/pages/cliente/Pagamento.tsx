...
import { ArrowLeft, CreditCard, Smartphone, QrCode, Receipt, DollarSign, Hash, Loader2, AlertTriangle } from 'lucide-react';
...
function PagamentoPage() {
  ...
  if (error || !pedido) {
    return <div className="p-6"><AlertTriangle className="h-8 w-8 text-red-500" /> <p>{error || "Pedido não encontrado."}</p></div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white shadow-sm border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link to={`/mesa/${numeroMesa}/acompanhar-pedido/${orderId}`} className="flex items-center text-gray-600 hover:text-gray-900 mr-4">
          ...
          <div className="space-y-3">
            {pedido.order_items.map(item => (
              <div key={item.id} className="flex justify-between items-center py-2 border-b border-gray-100">
                <p>{item.menu_items?.nome} x{item.quantidade}</p>
          ...
          {isProcessing ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {paymentMethods.map(method => (
                <button key={method.id} onClick={() => handlePaymentMethodSelection(method.id)} className="flex items-center justify-center gap-3 p-4 border-2 rounded-lg hover:border-primary-600">
      ...
      {/* Modal para PIX */}
      <Modal isOpen={isModalOpen && selectedMethod === 'pix'} onClose={() => setIsModalOpen(false)} titulo="Pagamento via PIX">
        <div className="text-center py-6">
          <p>Escaneie o QR Code ou use o código abaixo.</p>
          <div className="w-48 h-48 bg-gray-200 rounded-lg flex items-center justify-center mx-auto my-4"><QrCode className="h-24 w-24 text-gray-400" /></div>
      ...
      <Modal isOpen={isModalOpen && selectedMethod === 'dinheiro'} onClose={() => setIsModalOpen(false)} titulo="Pagamento no Caixa">
        <div className="text-center py-6">
          ...
          <button onClick={() => setIsModalOpen(false)} className="btn-primary">Entendi</button>
        </div>
      </Modal>
    </div>
  );
