...
const ReservasDashboard = () => {
  ...
  if (!data)
    return (
      <div className="text-center p-8">
        <AlertTriangle className="mx-auto h-8 w-8 text-red-500" />
        <p>Erro ao carregar dados de reservas.</p>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <MetricCard
          title="Taxa de Ocupação (Reservas)"
        ...
        <ul className="space-y-2">
          {(data.upcomingReservations || []).map((r, i) => (
            <li key={i} className="flex justify-between p-2 bg-gray-50 rounded">
              <span>
                {r.nome_cliente} ({r.pessoas}p)
              </span>
              <span className="font-bold">
                {new Date(r.data_hora).toLocaleTimeString([], {
                  hour: '2-digit',
                  minute: '2-digit',
                })}
              </span>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <MetricCard
          ...
          value={`R$ ${data.stockValue.toFixed(2)}`}
          icon={DollarSign}
        />
      </div>
      <div>
        ...
        <ul className="space-y-2">
          {(data.itemsNearExpiration || []).map((item, i) => (
            <li key={i} className="flex justify-between p-2 bg-yellow-50 rounded text-yellow-800">
              ...
              <span className="font-bold">
                Vence em: {new Date(item.validade).toLocaleDateString()}
              </span>
            </li>
          ))}
        </ul>
