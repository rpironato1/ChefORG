...
export const getSalesDashboardData = async (startDate: string, endDate: string): Promise<ApiResponse<SalesDashboardData>> => {
  try {
    ...
    if (!data) {
      return handleApiError(new Error('Nenhum dado encontrado'), 'Dados de vendas não encontrados.');
    }
...
export const getReservationsDashboardData = async (date: string): Promise<ApiResponse<ReservationsDashboardData>> => {
    try {
        ...
        if (error) throw error;
        if (!data) {
            return handleApiError(new Error('Nenhum dado encontrado'), 'Dados de reservas não encontrados.');
        }
        return createSuccessResponse(data);
    } catch (error) {
...
export const getStockDashboardData = async (): Promise<ApiResponse<StockDashboardData>> => {
    try {
        ...
        if (error) throw error;
        if (!data) {
            return handleApiError(new Error('Nenhum dado encontrado'), 'Dados de estoque não encontrados.');
        }
        return createSuccessResponse(data);
    } catch (error) {
...
export const getLoyaltyDashboardData = async (): Promise<ApiResponse<LoyaltyDashboardData>> => {
    try {
        ...
        if (error) throw error;
        if (!data) {
            return handleApiError(new Error('Nenhum dado encontrado'), 'Dados de fidelidade não encontrados.');
        }
        return createSuccessResponse(data);
    } catch (error) {
