...
export const useStock = () => {
  ...
  const adicionarItem = useCallback(
    (item: Omit<ItemEstoque, 'id' | 'ultimaAtualizacao'>): ItemEstoque => {
  ...
  const registrarDescarte = useCallback(
    (id: string, quantidade: number, motivo: string, funcionarioId: string): boolean => {
      const item = estoque.find(i => i.id === id);
      if (!item || item.quantidade < quantidade) return false;
  ...
  const alertarEstoqueBaixo = useCallback(
    (limite?: number): ItemEstoque[] => {
      return estoque.filter(item => item.quantidade <= (limite || item.estoqueMinimo));
    },
    [estoque]
  );

  const obterMovimentacoes = useCallback(
    (itemId?: string, periodo?: { inicio: Date; fim: Date }): MovimentacaoEstoque[] => {
      ...
      if (periodo) {
        resultado = resultado.filter(
          m => m.dataHora >= periodo.inicio && m.dataHora <= periodo.fim
        );
      }
  ...
  const calcularValorEstoque = useCallback((): number => {
    return estoque.reduce((total, item) => total + item.quantidade * item.valorUnitario, 0);
  }, [estoque]);
